...
async function cancelBooking(request, env, origin) {
  ...
  if (!email || !phone || (!bookingId && (!date || !startTime))) {
    return json({ error: "Please enter your email, mobile number and appointment date and time." }, 400, origin);
  }
  if (bookingId && !/^[0-9a-f-]{36}$/i.test(bookingId)) return json({ error: "Please check your booking reference." }, 400, origin);

  const booking = bookingId
    ? await env.DB.prepare("SELECT b.id,b.date,b.start_time,b.status,c.email,c.phone,c.first_name,c.surname FROM bookings b JOIN clients c ON c.id=b.client_id WHERE b.id=? LIMIT 1").bind(bookingId).first()
    : await env.DB.prepare("SELECT b.id,b.date,b.start_time,b.status,c.email,c.phone,c.first_name,c.surname FROM bookings b JOIN clients c ON c.id=b.client_id WHERE c.email=? AND b.date=? AND b.start_time=? ORDER BY b.created_at DESC LIMIT 1").bind(email, date, startTime).first();

  if (!booking) return json({ error: "We couldn't find that appointment. Please check your details and try again." }, 404, origin);
  if (booking.status !== "confirmed") return json({ error: booking.status === "cancelled" ? "This appointment has already been cancelled." : "This appointment can no longer be cancelled online." }, 409, origin);
  if (normaliseEmail(booking.email) !== email || normalisePhone(booking.phone) !== phone) return json({ error: "We couldn't verify those details. Please check your email and mobile number." }, 403, origin);
