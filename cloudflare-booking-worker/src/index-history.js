...
async function clientHistory(clientId, env, origin) {
  ...
  const history = (result.results || []).map(row => {
    ...
    return { id: row.id, date: row.date, startTime: row.start_time === "00:00" && metadata.source === "completed-treatment" ? null : row.start_time, services: serviceItems, addons: parseJson(row.addons_json, {}), originalPricePence: Number(row.price_pence || 0), priceAdjustmentPence: Number(row.price_adjustment_pence || 0), finalPricePence: Number(row.final_price_pence ?? row.price_pence ?? 0), paymentStatus: row.payment_status || "unpaid", status: row.status, source: metadata.source === "completed-treatment" ? "Completed treatment added by admin" : "Booking", notes: typeof metadata.notes === "string" ? metadata.notes : "" };
  });
