...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/admin/completed-treatment") {
        return admin(request, env, origin, () => addCompletedTreatment(request, env, origin));
      }
...
async function clientHistory(clientId, env, origin) {
  ...
  const history = (result.results || []).map(row => {
    ...
    return {
      id: row.id,
      date: row.date,
      startTime: row.start_time === "00:00" && metadata.source === "completed-treatment" ? null : row.start_time,
      services: serviceItems,
      addons: parseJson(row.addons_json, {}),
      originalPricePence: Number(row.price_pence || 0),
      priceAdjustmentPence: Number(row.price_adjustment_pence || 0),
      finalPricePence: Number(row.final_price_pence ?? row.price_pence ?? 0),
      paymentStatus: row.payment_status || "unpaid",
      status: row.status,
      source: metadata.source === "completed-treatment" ? "Completed treatment added by admin" : "Booking",
...
async function readSession(request, env) {
  ...
  try {
    const payload = fromB64url(encoded);
    if (!safeEqual(signature, await sign(payload, env.SESSION_SECRET))) return null;
    const i = payload.lastIndexOf("|"), email = payload.slice(0, i), expiry = Number(payload.slice(i + 1));
    if (!email || !Number.isFinite(expiry) || Date.now() > expiry) return null;
    if (env.ADMIN_EMAIL && email !== normaliseEmail(env.ADMIN_EMAIL)) return null;
    return { email, expiry };
  } catch { return null; }
}
