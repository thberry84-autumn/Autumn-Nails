...
async function adminLogin(request, env, origin) {
  ...
  if (email !== normaliseEmail(env.ADMIN_EMAIL) || password !== env.ADMIN_PASSWORD) {
    if (!record || now - record.firstAttempt >= LOGIN_WINDOW_MS) loginFailures.set(key, { failures: 1, firstAttempt: now });
    else loginFailures.set(key, { failures: record.failures + 1, firstAttempt: record.firstAttempt });
