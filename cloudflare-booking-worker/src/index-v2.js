...
async function addAvailability(request, env, origin) {
  ...
  const existing = await env.DB.prepare("SELECT start_time,status FROM availability_slots WHERE date = ? AND status IN ('available','booked')").bind(date).all();
