...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/admin/bookings/") && request.method === "PATCH") return requireAdmin(request, env, origin, () => updateBookingStatus(decodeURIComponent(url.pathname.split("/").pop()), request, env, origin));
      ...
      if (url.pathname === "/api/admin/calendar-token" && request.method === "POST") return requireAdmin(request, env, origin, () => rotateCalendarToken(env, origin));
...
async function publicAvailability(url, env, origin) {
  ...
  const result = await env.DB.prepare("SELECT id, date, start_time, service_ids_json FROM availability_slots WHERE status = 'available' AND removed_at IS NULL AND date >= date('now', 'localtime') ORDER BY date, start_time").all();
  ...
  }).map(row => ({ ...row, durationMinutes: APPOINTMENT_DURATION_MINUTES, endTime: addMinutesToTime(row.start_time, APPOINTMENT_DURATION_MINUTES) }));
...
async function createBooking(request, env, origin) {
  ...
  const slot = await env.DB.prepare("SELECT id, date, start_time, status, service_ids_json, removed_at FROM availability_slots WHERE id = ?").bind(slotId).first();
  if (!slot || slot.status !== "available" || slot.removed_at) return json({ error: "That appointment is no longer available. Please choose another slot." }, 409, origin);
  ...
  if (QUALIFYING_SERVICES[serviceId]) {
    ...
    if (!qualifyingDate || daysBetween(qualifyingDate, slot.date) > service.infillDays || daysBetween(qualifyingDate, slot.date) < 0) { effectiveService = SERVICES.find(s => s.id === service.fallbackId); infillChanged = true; }
  }
  ...
  statements.push(env.DB.prepare("UPDATE availability_slots SET status = 'booked', updated_at = ? WHERE id = ? AND status = 'available' AND removed_at IS NULL").bind(now, slotId));
  ...
  return json({ ok: true, booking: { id: bookingId, date: slot.date, startTime: slot.start_time, endTime: addMinutesToTime(slot.start_time, APPOINTMENT_DURATION_MINUTES), durationMinutes: APPOINTMENT_DURATION_MINUTES, service: effectiveService.name, requestedService: service.name, price: totalPrice, infillChanged, clientReturning: Boolean(existingClient) } }, 201, origin);
}
...
async function adminLogin(request, env, origin) {
  ...
  const body = await request.json(), email = normaliseEmail(body.email), password = String(body.password || ""), key = `${request.headers.get("CF-Connecting-IP") || "unknown"}:${email}`, now = Date.now(), record = loginFailures.get(key);
  if (record && now - record.firstAttempt < LOGIN_WINDOW_MS && record.failures >= LOGIN_MAX_FAILURES) return json({ error: "Too many login attempts. Please try again later." }, 429, origin, { "Retry-After": String(Math.ceil((LOGIN_WINDOW_MS - (now - record.firstAttempt)) / 1000)) });
  if (email !== normaliseEmail(env.ADMIN_EMAIL) || password !== env.ADMIN_PASSWORD) { if (!record || now - record.firstAttempt >= LOGIN_WINDOW_MS) loginFailures.set(key, { failures: 1, firstAttempt: now }); else loginFailures.set(key, { failures: record.failures + 1, firstAttempt: record.firstAttempt }); return json({ error: "Incorrect email or password." }, 401, origin); }
  loginFailures.delete(key); const payload = `${email}|${Date.now() + ADMIN_SESSION_MAX_AGE * 1000}`, signature = await sign(payload, env.SESSION_SECRET); return json({ ok: true, token: `${b64url(payload)}.${signature}` }, 200, origin);
}
...
async function adminAvailability(env, origin) { const result = await env.DB.prepare("SELECT id, date, start_time, service_ids_json, status, removed_at, created_at FROM availability_slots ORDER BY date, start_time").all(); return json({ slots: (result.results || []).map(row => ({ ...row, serviceIds: parseJson(row.service_ids_json, []), durationMinutes: APPOINTMENT_DURATION_MINUTES, endTime: addMinutesToTime(row.start_time, APPOINTMENT_DURATION_MINUTES), displayStatus: row.removed_at ? "removed" : row.status })) }, 200, origin, { "Cache-Control": "no-store" }); }
async function addAvailability(request, env, origin) {
  const body = await request.json(), date = cleanDate(body.date), startTime = cleanTime(body.startTime), serviceIds = Array.isArray(body.serviceIds) ? body.serviceIds.filter(id => SERVICES.some(s => s.id === id && s.bookable)) : [];
  if (!date || !startTime) return json({ error: "Please provide a date and time." }, 400, origin);
  const existing = await env.DB.prepare("SELECT start_time,status FROM availability_slots WHERE date = ? AND status IN ('available','booked') AND removed_at IS NULL").bind(date).all();
  const conflict = (existing.results || []).some(row => Math.abs(minutesFromMidnight(row.start_time) - minutesFromMidnight(startTime)) < APPOINTMENT_DURATION_MINUTES);
  if (conflict) return json({ error: "That time overlaps another 2-hour appointment space." }, 409, origin);
  const now = new Date().toISOString(); const result = await env.DB.prepare("INSERT INTO availability_slots (date, start_time, service_ids_json, status, created_at, updated_at) VALUES (?, ?, ?, 'available', ?, ?)").bind(date, startTime, JSON.stringify(serviceIds), now, now).run(); return json({ ok: true, id: result.meta.last_row_id, durationMinutes: APPOINTMENT_DURATION_MINUTES, endTime: addMinutesToTime(startTime, APPOINTMENT_DURATION_MINUTES) }, 201, origin);
}
async function deleteAvailability(id, env, origin) { if (!Number.isInteger(id)) return json({ error: "Invalid slot." }, 400, origin); const row = await env.DB.prepare("SELECT status,removed_at FROM availability_slots WHERE id = ?").bind(id).first(); if (!row) return json({ error: "Slot not found." }, 404, origin); if (row.status === "booked") return json({ error: "A booked slot cannot be deleted. Cancel the booking instead." }, 409, origin); if (row.removed_at) return json({ error: "Slot is already removed." }, 409, origin); await env.DB.prepare("UPDATE availability_slots SET removed_at=?,updated_at=? WHERE id=?").bind(new Date().toISOString(), new Date().toISOString(), id).run(); return json({ ok: true, removed: true }, 200, origin); }
async function adminBookings(env, origin) { const result = await env.DB.prepare("SELECT b.id,b.date,b.start_time,b.price_pence,b.status,b.service_id,b.booked_service_id,b.addons_json,c.first_name,c.surname,c.email,c.phone FROM bookings b JOIN clients c ON c.id=b.client_id ORDER BY b.date DESC,b.start_time DESC").all(); return json({ bookings: (result.results || []).map(row => ({ ...row, requestedService: serviceName(row.service_id), bookedService: serviceName(row.booked_service_id), addons: parseJson(row.addons_json, {}), durationMinutes: APPOINTMENT_DURATION_MINUTES, endTime: addMinutesToTime(row.start_time, APPOINTMENT_DURATION_MINUTES) })) }, 200, origin, { "Cache-Control": "no-store" }); }
async function updateBookingStatus(id, request, env, origin) { if (!id || typeof id !== "string") return json({ error: "Invalid booking." }, 400, origin); const status = (await request.json()).status; if (!["confirmed","completed","cancelled"].includes(status)) return json({ error: "Invalid status." }, 400, origin); const booking = await env.DB.prepare("SELECT slot_id FROM bookings WHERE id = ?").bind(id).first(); if (!booking) return json({ error: "Booking not found." }, 404, origin); const now = new Date().toISOString(); await env.DB.batch([env.DB.prepare("UPDATE bookings SET status=?,updated_at=? WHERE id=?").bind(status, now, id), env.DB.prepare("UPDATE availability_slots SET status=?,removed_at=CASE WHEN ?='cancelled' THEN NULL ELSE removed_at END,updated_at=? WHERE id=?").bind(status === "cancelled" ? "available" : "booked", status, now, booking.slot_id), env.DB.prepare("INSERT INTO booking_events (id,booking_id,event_type,metadata_json,created_at) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), id, status, JSON.stringify({ source: "admin" }), now)]); return json({ ok: true }, 200, origin); }
...
async function rotateCalendarToken(env, origin) { const token = crypto.randomUUID().replace(/-/g, "") + crypto.randomUUID().replace(/-/g, ""); await env.DB.prepare("INSERT INTO settings (key,value,updated_at) VALUES ('calendar_token',?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at").bind(token, new Date().toISOString()).run(); return calendarToken(env, origin); }
async function calendarFeed(token, env) { const expected = await env.DB.prepare("SELECT value FROM settings WHERE key='calendar_token'").first(); if (!expected || token.replace(/\.ics$/, "") !== expected.value) return new Response("Not found", { status: 404, headers: { "Cache-Control": "no-store" } }); const result = await env.DB.prepare("SELECT b.id,b.date,b.start_time,b.status,b.booked_service_id,c.first_name FROM bookings b JOIN clients c ON c.id=b.client_id WHERE b.status IN ('confirmed','completed') ORDER BY b.date,b.start_time").all(); const stamp = toIcsDateTime(new Date().toISOString().slice(0,10), new Date().toISOString().slice(11,16)); const events=(result.results||[]).map(row=>["BEGIN:VEVENT",`UID:booking-${row.id}@autumnnails.com`,`DTSTAMP:${stamp}Z`,`DTSTART;TZID=Europe/London:${toIcsDateTime(row.date,row.start_time)}`,`DTEND;TZID=Europe/London:${toIcsDateTime(row.date,addMinutesToTime(row.start_time,APPOINTMENT_DURATION_MINUTES))}`,`SUMMARY:${escapeIcs(`Autumn Nails – ${row.first_name}`)}`,`DESCRIPTION:${escapeIcs(serviceName(row.booked_service_id))}`,"END:VEVENT"].join("\r\n")); const ics=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Autumn Nails//Booking Calendar//EN","CALSCALE:GREGORIAN","X-WR-CALNAME:Autumn Nails",...events,"END:VCALENDAR",""] .join("\r\n"); return new Response(ics,{status:200,headers:{"Content-Type":"text/calendar; charset=utf-8","Cache-Control":"no-store","Content-Disposition":"inline; filename=autumn-nails.ics","X-Content-Type-Options":"nosniff","X-Frame-Options":"DENY","Content-Security-Policy":"default-src 'none'; frame-ancestors 'none'; base-uri 'none'"}}); }
function serviceName(id){return SERVICES.find(s=>s.id===id)?.name||id;} function normaliseAddons(value){if(!value||typeof value!=="object")return{};const r={};for(const s of SERVICES.filter(x=>x.addon)){const q=Math.max(0,Math.min(10,Number(value[s.id]||0)));if(Number.isFinite(q)&&q>0)r[s.id]=Math.floor(q);}return r;} function parseJson(value,fallback){try{return JSON.parse(value||"")}catch{return fallback}} function cleanText(value,max){return String(value||"").trim().replace(/\s+/g," ").slice(0,max)} function normaliseEmail(value){return String(value||"").trim().toLowerCase().slice(0,254)} function cleanDate(value){const text=String(value||"");return /^\d{4}-\d{2}-\d{2}$/.test(text)?text:""} function cleanTime(value){const text=String(value||"");return /^(?:[01]\d|2[0-3]):[0-5]\d$/.test(text)?text:""} function daysBetween(from,to){return Math.round((Date.parse(`${to}T00:00:00Z`)-Date.parse(`${from}T00:00:00Z`))/86400000)} function minutesFromMidnight(time){const [h,m]=time.split(":").map(Number);return h*60+m} function addMinutesToTime(time,minutes){const total=minutesFromMidnight(time)+minutes;return `${String(Math.floor(total/60)%24).padStart(2,"0")}:${String(total%60).padStart(2,"0")}`} function toIcsDateTime(date,time){return `${date.replace(/-/g,"")}T${time.replace(":","")}00`} function escapeIcs(value){return String(value).replace(/\\/g,"\\\\").replace(/\r?\n/g,"\\n").replace(/([,;])/g,"\\$1")}
