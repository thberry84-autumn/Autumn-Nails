(() => {
  ...
  const setSlotDate=iso=>{const el=document.getElementById('slotDate');if(!el)return;el.type='date';el.inputMode='';el.placeholder='';el.value=iso};
  const setSlotTime=value=>{const el=document.getElementById('slotTime');if(!el)return;el.value=value;const button=el.closest('.time-picker-wrap')?.querySelector('.time-picker-button');if(button)button.textContent=value||'Choose time';el.dispatchEvent(new Event('change',{bubbles:true}))};
  ...
  const jumpToAvailability=(date,time)=>{state.selected=parseDate(date)||new Date();setSlotDate(date);setSlotTime(time);scrollToSlot()};
  ...
  function install(){
    ...
    if(date){const current=readSlotDate(date.value)||dateKey(new Date());date.type='date';date.inputMode='';date.placeholder='';date.value=current}
    return true;
  }
  ...
  function render(){
    ...
    grid.querySelectorAll('[data-empty-date]').forEach(el=>{el.onclick=()=>{state.selected=parseDate(el.dataset.emptyDate);setSlotDate(el.dataset.emptyDate);scrollToSlot()};el.ondblclick=()=>{const hour=Number(el.style.top||0)/60;const time=`${pad(9+Math.floor(hour))}:00`;jumpToAvailability(el.dataset.emptyDate,time)}});
  }
  function dayColumn(d,all){const key=dateKey(d);const items=all.filter(x=>x.date===key).sort((a,b)=>mins(a.start_time)-mins(b.start_time));return `<div class="cal-day ${key===dateKey(new Date())?'today':''}">${Array.from({length:14},(_,i)=>`<div class="cal-hour" data-empty-date="${key}" style="top:${i*60}px"></div>`).join('')}${items.map(x=>{const top=Math.max(0,(mins(x.start_time)-DAY_START_MINUTES)/60);if(x._type==='booked')return `<button type="button" class="cal-event booked${eventClass(x.status)}" data-booking-id="${esc(x.id)}" style="top:${top}px"><b>${esc(fmtTime(x.start_time))}</b><strong>${esc(bookingName(x))}</strong><span>${esc(services(x))}</span><small>${esc(x.status||'confirmed')} · ${money(x.finalPricePence??x.price_pence)}</small></button>`;return `<button type="button" class="cal-event available" data-available-id="${esc(x.id)}" style="top:${top}px"><b>${esc(fmtTime(x.start_time))}</b><strong>Available</strong><span>${esc((x.services||x.service_ids||[]).join(', ').replaceAll('-',' '))}</span></button>`}).join('')}</div>`}
