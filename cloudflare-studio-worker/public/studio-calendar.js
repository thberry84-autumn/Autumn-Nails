(() => {
  ...
  function render(){
    ...
    const summary=document.querySelector('[data-cal-summary]');
    if(!grid)return;
    ...
    const booked=all.filter(x=>x._type==='booked'&&x.status!=='cancelled').length;
    const available=all.filter(x=>x._type==='available'&&x.status!=='booked').length;
