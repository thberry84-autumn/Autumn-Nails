...
export default {
  async fetch(request, env) {
    ...
    const scripts = '<script src="/studio-actions.js?v=20260904f"></script><script src="/studio-calendar.js?v=20260904f"></script><script src="/studio-booking-fixes.js?v=20260904f"></script><script>(function(){const clean=()=>document.querySelectorAll("style#studio-polish-css,style:not([id])").forEach(s=>{if(s.id==="studio-polish-css"||s.textContent.includes("#studioCalendar{"))s.remove()});new MutationObserver(clean).observe(document.head,{childList:true});clean()})();</script><script>(function(){function enhance(){const host=document.getElementById("financeTable");const table=host?.querySelector("table");if(!table||table.dataset.amendReady==="1")return;const head=table.querySelector("thead tr");if(head){const th=document.createElement("th");th.textContent="Actions";head.appendChild(th)}table.querySelectorAll("tbody tr").forEach(row=>{const cells=row.querySelectorAll("td");if(cells.length<6)return;const idCell=cells[0];const date=idCell.textContent.trim();const amend=document.createElement("button");amend.type="button";amend.className="button secondary";amend.textContent="Amend";amend.addEventListener("click",()=>{if(row.querySelector(".finance-amend"))return;const original=cells[2].textContent.trim().replace("£","");const adjustment=cells[3].textContent.trim().replace("£","");const payment=cells[5].textContent.trim();const originalValue=Number(original)||0;const wrap=document.createElement("div");wrap.className="finance-amend";wrap.style.marginTop="8px";wrap.innerHTML=`<div style="display:grid;gap:6px;min-width:170px"><label style="font-size:.68rem;text-transform:uppercase;letter-spacing:.1em">Adjustment £</label><input type="number" step="0.01" value="${adjustment}"><label style="font-size:.68rem;text-transform:uppercase;letter-spacing:.1em">Payment</label><select><option value="unpaid">Unpaid</option><option value="paid">Paid</option><option value="refunded">Refunded</option><option value="not-required">Not required</option></select><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px"><button type="button" class="button">Save</button><button type="button" class="button secondary">Cancel</button></div><span class="msg"></span></div>`;const input=wrap.querySelector("input"),select=wrap.querySelector("select"),save=wrap.querySelector("button:not(.secondary)"),cancel=wrap.querySelector("button.secondary"),message=wrap.querySelector(".msg");const match=["unpaid","paid","refunded","not-required"].find(v=>v===payment)||"unpaid";select.value=match;save.addEventListener("click",async()=>{const value=Math.round((Number(input.value)||0)*100);if(!Number.isFinite(value)||value<-100000||value>100000){message.textContent="Please enter a valid adjustment.";return}save.disabled=true;message.textContent="Saving…";try{const d=await api(BOOKING_API,"/api/bookings/"+encodeURIComponent(row.dataset.bookingId||""),{method:"PATCH",body:JSON.stringify({priceAdjustmentPence:value,paymentStatus:select.value})});if(!row.dataset.bookingId)throw Error("Booking ID unavailable.");const final=Math.max(0,originalValue*100+value);cells[3].textContent=money(value);cells[4].textContent=money(final);cells[5].textContent=select.options[select.selectedIndex].text;wrap.remove();amend.disabled=false;loadFinance()}catch(e){message.textContent=e.message;save.disabled=false}});cancel.addEventListener("click",()=>wrap.remove());cells[6].appendChild(wrap)});const rows=table.querySelectorAll("tbody tr");rows.forEach((row,index)=>{if(row.dataset.amendAction==="1")return;const bookingId=host.querySelector(`[data-finance-row-id="${index}"]`)?.dataset.bookingId||"";row.dataset.amendAction="1";row.dataset.bookingId=bookingId;const cell=document.createElement("td");cell.appendChild(amend);row.appendChild(cell)});table.dataset.amendReady="1"}const observer=new MutationObserver(enhance);observer.observe(document.body,{childList:true,subtree:true});enhance()})();</script>';
