...
export async function handleStudioMutation(request,env,ctx,origin,pathname){
  ...
  if(pathname.startsWith("/api/bookings/")&&pathname.endsWith("/cancel")){
    ...
    const current=await env.DB.prepare("SELECT id,status,slot_id,payment_status,price_adjustment_pence,price_pence,final_price_pence FROM bookings WHERE id=? LIMIT 1").bind(id).first();
