...
async function uploadFiles(request, env, origin) {
  ...
  await saveCaptions(metadata, env);
...
async function listGallery(env) {
  ...
  return listed.objects
    ...
    .map(o => ({ name: o.key, caption: captions[o.key] || "", url: `https://autumn-nails-upload.thberry84.workers.dev/images/${encodeURIComponent(o.key)}` }));
}
...
async function saveCaptions(captions, env) {
  ...
  const body = { message: "Update nail photo captions", content };
  if (sha) body.sha = sha;
...
async function githubRequest(path, token, options = {}) {
  return fetch(`${GITHUB_API}${path}`, {
    ...options,
    headers: { Accept: "application/vnd.github+json", Authorization: `Bearer ${token}`, "X-GitHub-Api-Version": "2026-03-10", "User-Agent": "Autumn-Nails-Upload-Worker", ...(options.headers || {}) }
  });
}
